    ...
    private func functionDemonstrator(label: String, input: String, output: String) -> some View {
        VStack(alignment: .leading) {
            ...
            HStack {
                ...
                Spacer()
                UniversalText(input, size: Constants.UISmallTextSize, font: FontProvider[.renoMono])
                Spacer()
                ResizeableIcon("arrow.right", size: Constants.UIDefaultTextSize)
                Spacer()
                UniversalText(output, size: Constants.UISmallTextSize, font: FontProvider[.renoMono])
                Spacer()
            }
        }
        .padding(.bottom, 7)
